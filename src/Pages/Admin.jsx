...
function CompanyForm({ onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.companyName || !formData.symbol || !formData.stockPrice || !formData.totalShares) {
  ...
  return (
    <div className="p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          <label className="block text-gray-300 mb-2 font-medium">Stock Price ($)</label>
          <input
            type="number"
            name="stockPrice"
        ...
        <div>
          <label className="block text-gray-300 mb-2 font-medium">Total Shares</label>
          <input
            type="number"
            name="totalShares"
...
export default function Admin() {
  ...
  const handleDeleteCompany = async (companyId) => {
    if (window.confirm("Are you sure you want to delete this company?")) {
      try {
        const response = await fetch(`http://localhost:4000/api/admin/companies/${companyId}`, {
          method: "DELETE",
          headers: {
            'Content-Type': 'application/json',
          }
                ...
                <table className="w-full bg-gray-900/70">
                  <thead className="bg-gray-800/70">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-center text-gray-300 font-semibold">Email</th>
                      <th className="px-6 py-4 text-right text-gray-300 font-semibold">Balance</th>
                      <th className="px-6 py-4 text-right text-gray-300 font-semibold">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-800/50">
                    {users.map((user) => (
                      <tr key={user.user_id} className="hover:bg-gray-800/40 transition-colors duration-200">
                        <td className="px-6 py-4 text-left text-gray-200 font-medium">{user.name}</td>
                        <td className="px-6 py-4 text-center text-gray-300">{user.email}</td>
                        <td className="px-6 py-4 text-left text-blue-400 font-medium">${user.balance}</td>
              ...
              <div className="overflow-x-auto rounded-lg border border-gray-700/50">
                <table className="w-full bg-gray-900/70">
                  <thead className="bg-gray-800/70">
                    <tr>
                      <th className="px-6 py-4 text-left text-gray-300 font-semibold">Name</th>
                      <th className="px-6 py-4 text-center text-gray-300 font-semibold">Symbol</th>
                      <th className="px-6 py-4 text-right text-gray-300 font-semibold">Stock Price</th>
                      <th className="px-6 py-4 text-right text-gray-300 font-semibold">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-800/50">
                    {companies.map((company) => (
                      <tr key={company.company_id} className="hover:bg-gray-800/40 transition-colors duration-200">
                        <td className="px-6 py-4 text-gray-200 text-left font-medium">{company.company_name}</td>
                        <td className="px-6 py-4 text-blue-400 font-medium">{company.symbol}</td>
                        <td className="px-6 py-4 text-green-400 font-medium">${company.stock_price}</td>
